...
func NewArchiveReaderSystem(archivePath string, data []byte, format archive.Format, options ArchiveReaderSystemOptions) (*ArchiveReaderSystem, error) {
	...
	if format == archive.FormatUnknown {
		format = archive.GuessFormat(archivePath, data)
	}
